...
try:
    ...
    st.error(
        "Biblioteca reportlab não está instalada. Instale-a com: `pip install reportlab`"
    )
    raise e
...
st.set_page_config(
    page_title="GERENCIAMENTO DE RISCO", page_icon=":warning:", layout="wide"
)
...
st.markdown(
    "<h1 style='text-align: center; color: white;'>GERENCIAMENTO DE RISCO ⚠️</h1>",
    unsafe_allow_html=True,
)
ativo = st.selectbox("Ativo Operado:", list(futuros.keys()))
tipo_operacao = st.selectbox("Posição Aberta:", ["COMPRA", "VENDA"])
preco_abertura_posicao = float(
    st.number_input("Preço Abertura de Posição:", format="%.2f")
)
...
if ativo == "MICRO NASDAQ":
    total_contratos = int(
        st.number_input("Quantidade Contratos:", min_value=0, max_value=85)
    )
elif ativo == "MINI NASDAQ":
    total_contratos = int(
        st.number_input("Quantidade Contratos:", min_value=0, max_value=17)
    )
    ...
    total_contratos = float(
        st.number_input(
            "Quantidade Fracionada da Onças Troy:", min_value=0.01, max_value=1.000
        )
    )
    ...
    total_contratos = float(
        st.number_input(
            "Quantidade Fracionada do Barril:", min_value=0.01, max_value=1.000
        )
    )
...
def calculate():
    ...
    # Exibe os resultados
    st.write(
        f"Nesta {tipo_operacao.upper()} o seu ALVO é de {abs(alvo):.2f} pontos, com LUCRO de US$ {lucro:.2f}."
    )
    st.write(
        f"Nesta {tipo_operacao.upper()} o seu STOP é de {abs(stop):.2f} pontos, com PERDA de US$ {perda:.2f}."
    )
...
def generate_pdf(resultados):
    try:
        ...
        c.drawCentredString(
            width / 2, height - 50, "Relatório de Gerenciamento de Risco"
        )
...
if st.button("Calcular"):
    if total_contratos == 0:
        st.error(
            "Por favor, selecione uma quantidade de contratos válida antes de calcular."
        )
    else:
        try:
            calculate()
